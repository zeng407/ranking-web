...
@section('content')
  <game inline-template post-serial="{{ $serial }}" :user-last-game="{{ json_encode($userLastGame) }}"
    :require-password="{{ json_encode($requiredPassword) }}" get-rank-route="{{ route('game.rank', '_serial') }}"
    get-game-setting-endpoint="{{ route('api.game.setting', $serial) }}"
    next-round-endpoint="{{ route('api.game.next-round', '_serial') }}"
    create-game-endpoint="{{ route('api.game.create') }}" vote-game-endpoint="{{ route('api.game.vote') }}"
    access-endpoint="{{ route('api.game.access', $serial) }}" props-game-room-serial="{{ $gameRoom?->serial }}"
    ...
    <div class="container-fluid hide-scrollbar pt-2" v-cloak>
      ...
      @if (!$post->is_censored)
        <div v-show="game && !creatingGame && !finishingGame">
          @include('ads.gam_togawa_300x250', ['displayEvent' => 'ranking:game-ads-ready'])
        </div>
      @endif
      ...
      @if (
          !$post->is_censored &&
              ...
              config('services.google_ad.game_page') &&
              !is_skip_ad())
        {{-- ads --}}
        ...
        <div v-if="isMobileScreen" id="google-ad-container" style="height: 100px; z-index:1"
      ...
      <div v-show="creatingGame">
        <div class="d-flex justify-content-center align-items-center flex-column" style="height: 100vh">
          <img src="{{ asset('storage/logo.png') }}" class="updown-animation" alt="logo"
      ...
      <div v-show="finishingGame && !isBetGameClient" class="finishing-game-overlay"
      ...
      <div class="row">
        ...
        <div class="col-xl-2 col-xl-fixed-280 d-none d-xl-block">
          ...
          <div v-if="showMatchHistory" class="match-history-list d-flex flex-column py-2" style="height: 850px">
            ...
            <transition-group name="match-list" tag="div" class="d-flex flex-column overflow-y-scroll hide-scrollbar"
              style="flex: 1; min-height: 0;">
              <div v-for="item in matchHistory" :key="item.id" class="match-card mb-2"
                :class="item.winSide === 'left' ? 'left-win' : 'right-win'" style="font-size: 0.75rem;">
                ...
                <div class="position-relative mb-1">
                  <img :src="item.winner.thumb" :alt="item.winner.title" class="w-100 bg-dark user-select-none"
                    style="height: 100px; object-fit: cover; border-radius: 4px;">
                  <div class="position-absolute w-100"
                    style="bottom: 0; background: linear-gradient(transparent, rgba(0,0,0,0.8)); padding: 2px 4px;">
                ...
                <div class="position-relative" style="opacity: 0.6;">
                  <img :src="item.loser.thumb" :alt="item.loser.title" class="w-100 bg-dark user-select-none"
                    style="height: 100px; object-fit: cover; border-radius: 4px;">
                  <div class="position-absolute w-100"
                    style="bottom: 0; background: linear-gradient(transparent, rgba(0,0,0,0.8)); padding: 2px 4px;">
                    <div class="d-flex align-items-center">
        ...
        <div class="col-xl col-12">
          ...
          <transition-group name="bet-result-animation" tag="div"
            class="position-absolute top-50 left-50 translate-50-50 w-100 bg-dark-50 overflow-hidden"
            style="z-index: 110">
            ...
            <div class="d-none d-sm-flex" style="flex-flow: row wrap">
              ...
              <h3 class="d-flex justify-content-end text-right align-self-center" style="width: 20%">
                <div class="badge badge-pill badge-light px-3 py-1 shadow-sm d-flex align-items-center mr-2"
                  style="font-size: 0.85rem; border: 1px solid #dee2e6; max-width: 200px" title="Timer">
          ...
          <div class="row overflow-hidden user-select-none"
              ...
              <transition name="slide-in-left">
                <div v-if="animationShowLeftPlayer" class="card game-player left-player vs-card" id="left-player">
                  <div v-show="isImageSource(le)" class="game-image-container" v-cloak>
                    <flex-image :key="le.id" v-show="!leftImageLoaded" :element-id="le.id"
                      :thumb-url="getLowThumbUrl(le)" :image-key="le.id" :imgur-url="le.imgur_url"
                      :alt="le.title" :height="elementHeight"></flex-image>
                    <viewer ref="leftViewer" :options="viewerOptions">
                      <flex-image :key="le.id" v-show="leftImageLoaded" :handle-loaded="handleLeftLoaded"
                        :element-id="le.id" :thumb-url="getThumbUrl(le)" :image-key="le.id"
                        :imgur-url="le.imgur_url" :alt="le.title" :height="elementHeight"></flex-image>
                    </viewer>
                  </div>
                  <div v-if="isYoutubeSource(le) && !isDataLoading" class="d-flex hover-effect"
                    @mouseover="videoHoverIn(le, re, true)" @mouseleave="videoHoverOut(le, re, true)">
                    <youtube :video-id="le.video_id" width="100%" :height="elementHeight" :ref="le.id"
                      :player-vars="{ controls: 1, autoplay: !isMobileScreen && !isBetGameClient, rel: 0, origin: origin, playlist: le
                              .video_id, start: le.video_start_second, end: le.video_end_second }">
                    </youtube>
                  </div>
                  ...
                  <div v-else-if="isBilibiliSource(le) && !isDataLoading" class="d-flex">
                    <bilibili-video v-if="le" :element="le" width="100%" :autoplay="false"
                      :muted="false" :height="elementHeight" />
                  </div>
                  ...
                  <div v-else-if="isTwitchClipSource(le) && !isDataLoading" class="d-flex twitch-container">
                    <iframe :src="'https://clips.twitch.tv/embed?clip=' + le.video_id + '&parent=' + host + '&autoplay=false'"
                      :height="elementHeight" width="100%" allowfullscreen></iframe>
                  </div>
                  <div v-else-if="isVideoSource(le) && !isDataLoading" class="d-flex">
                    <video id="left-video-player" v-if="isMobileScreen" width="100%" :height="elementHeight" loop
                      controls playsinline :src="le.source_url" :poster="le.thumb_url"></video>
                    <video id="left-video-player" v-else width="100%" :height="elementHeight" loop controls
                      playsinline muted :src="le.source_url" :poster="le.thumb_url"></video>
                  </div>
                  <div class="card-body text-center">
                    ...
                    <button id="left-btn" class="btn btn-vote-left vote-button btn-block d-none d-sm-block"
                      ...
                      <h3 class="d-inline-block m-0" v-if="showGameRoomVotes">
                        <I-Count-Up :end-val="leftVotes" :options="{ 'duration': 0.5 }"
                          :delay="0"></I-Count-Up>
                        (<I-Count-Up :end-val="leftVotesPercentage" :options="{ 'duration': 0.5 }"
                          :delay="0"></I-Count-Up>%)
                      </h3>
                    </button>
                    <div class="row" v-if="isYoutubeSource(le) || isVideoUrlSource(le)">
                      {{-- mobile --}}
                      <div class="col-3">
                        <button class="btn btn-outline-primary btn-lg btn-block d-block d-sm-none btn-hover-bounce"
                          :class="{ active: isLeftPlaying }" :disabled="isVoting" @click="leftPlay()">
            ...
            <div class="col-12 col-sm-6 pl-sm-1 mb-4 mb-sm-0" id="right-part">
              <transition :name="isMobileScreen ? 'slide-in-left' : 'slide-in-right'">
                <div v-if="animationShowRightPlayer" class="card game-player right-player vs-card" id="right-player"
                  :class="{ 'flex-column-reverse': isMobileScreen, 'mb-4': isMobileScreen }">
                  <div v-show="isImageSource(re)" class="game-image-container" v-cloak>
                    <flex-image v-show="!rightImageLoaded" :key="re.id" :element-id="re.id"
                      :thumb-url="getLowThumbUrl(re)" :image-key="re.id" :imgur-url="re.imgur_url"
                      :alt="re.title" :height="elementHeight"></flex-image>
                    <viewer ref="rightViewer" :options="viewerOptions">
                      <flex-image v-show="rightImageLoaded" :key="re.id" :handle-loaded="handleRightLoaded"
                        :element-id="re.id" :thumb-url="getThumbUrl(re)" :image-key="re.id"
                        :imgur-url="re.imgur_url" :alt="re.title" :height="elementHeight"></flex-image>
                    </viewer>
                  </div>
                  <div v-if="isYoutubeSource(re) && !isDataLoading" class="d-flex hover-effect"
                    @mouseover="videoHoverIn(re, le, false)" @mouseleave="videoHoverOut(re, le, false)">
                    <youtube :video-id="re.video_id" width="100%" :height="elementHeight" :ref="re.id"
                      :player-vars="{ controls: 1, autoplay: !isMobileScreen && !isBetGameClient, rel: 0, origin: origin, playlist: re
                              .video_id, start: re.video_start_second, end: re.video_end_second }">
                    </youtube>
                  </div>
                  ...
                  <div v-else-if="isBilibiliSource(re) && !isDataLoading" class="d-flex">
                    <bilibili-video v-if="re" :element="re" width="100%" :autoplay="false"
                      :muted="false" :height="elementHeight" />
                  </div>
                  <div v-else-if="isVideoSource(re) && !isDataLoading" class="d-flex">
                    <video id="right-video-player" v-if="isMobileScreen" width="100%" :height="elementHeight" loop
                      controls playsinline :src="re.source_url" :poster="re.thumb_url"></video>
                    <video id="right-video-player" v-else width="100%" :height="elementHeight" loop controls
                      playsinline muted :src="re.source_url" :poster="re.thumb_url"></video>
                  </div>

                  <!-- reverse when device size width less md(768px)-->
                  <div class="card-body text-center"
                    :class="{ 'flex-column-reverse': isMobileScreen, 'd-flex': isMobileScreen }">
                    <div class="my-1 flex-column-reverse d-flex overflow-scroll hide-scrollbar" style="max-height: 35px"
                      v-if="isMobileScreen">
                    ...
                    <button id="right-btn" class="btn btn-vote-right vote-button btn-block d-none d-sm-block"
                      ...
                      <h3 class="d-inline-block m-0" v-if="showGameRoomVotes">
                        <I-Count-Up :end-val="rightVotes" :options="{ 'duration': 0.5 }"
                          :delay="0"></I-Count-Up>
                        (<I-Count-Up :end-val="rightVotesPercentage" :options="{ 'duration': 0.5 }"
                          :delay="0"></I-Count-Up>%)
                      </h3>
                    </button>
                    <div class="row" v-if="isYoutubeSource(re) || isVideoUrlSource(re)">
                      {{-- mobile --}}
                      <div class="col-3">
                        <button class="btn btn-outline-danger btn-lg btn-block d-block d-sm-none btn-hover-bounce"
                          :class="{ active: isRightPlaying }" :disabled="isVoting" @click="rightPlay()">
          ...
          <div class="game-room-box" style="height: 800px;">
            <div class="game-room-container">
              ...
              <div class="p-1 my-1" v-if="isBetGameClient">
                <input v-show="isEditingNickname" id="newNickname" autocomplete="off" v-model="newNickname"
                  type="text" class="form-control badge-secondary bg-secondary-onfocus font-size-16" maxlength="10">
                <div class="text-center" v-show="isEditingNickname">(@{{ $t('game_room.nickname_hint') }})</div>
                <h5 class="d-flex justify-content-center">
                  <div class="position-relative">
                    <u class="cursor-pointer" v-if="!isEditingNickname"
                      @click="toggleEditNickname">@{{ gameRoom.user.name | cut(10) }}</u>
                    {{-- edit name --}}
                    <button v-if="!isEditingNickname && !gameRoom.is_game_completed"
                      class="btn btn-secondary btn-sm position-absolute m-0 p-1 px-1" @click="toggleEditNickname">
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <span>
                    <I-Count-Up :end-val="Number(gameRoom.user.accuracy)"
                      :options="{ suffix: '%', decimalPlaces: '2' }"></I-Count-Up>
                  </span>
                </div>
                <div class="d-flex justify-content-between">
              ...
              <div class="p-1 my-1">
                <h5 class="text-center position-relative">
                  ...
                  <span v-show="sortByTop" class="btn btn-secondary btn-sm cursor-pointer position-absolute m-0 p-1"
                  ...
                  <span v-if="isBetGameHost" class="btn btn-sm btn-secondary cursor-pointer position-absolute p-0"
                    ...
                    id="minimize-game-room" style="right:20px" @click="minimizeGameRoom">
                    <i class="fa-solid fa-minus"></i>
                  </span>
                  <span v-if="isBetGameHost" class="btn btn-sm btn-secondary cursor-pointer position-absolute p-0"
                    data-toggle="tooltip" data-placement="left" :title="$t('game_room.close_game')"
                    id="close-game-room" style="right:0" @click="closeGameRoom">
                    <i class="fa-solid fa-xmark"></i>
                  </span>
                </h5>
                <h5 class="d-flex justify-content-between bet-rank-broad">
                  ...
                  <span>@{{ $t('game_room.point') }}
                    <i class="fa-solid fa-circle-question" data-toggle="tooltip" data-placement="left"
                      data-html="true" :title="$t('game_room.point_description')"></i>
                  </span>
                </h5>
                <hr class="my-1 text-white bg-white">
                <transition-group name="list-left" tag="div">
                  <div class="d-flex justify-content-between position-relative align-items-center" v-if="getSortedRanks"
                    v-for="(rank,index) in getSortedRanks" :key="rank.user_id + ':' + rank.rank">
                    <h5 class="d-flex align-items-center bet-rank-broad">
                      ...
                      <i v-else-if="rank.rank > 0 && rank.rank == gameBetRanks.total_users" class="fa-solid fa-poo mr-1"
                        style="color:gold"></i>
                      <i v-else-if="rank.rank > 0 && rank.rank == gameBetRanks.total_users -1"
                        class="fa-solid fa-poo mr-1" style="color:silver"></i>
                      <i v-else-if="rank.rank > 0 && rank.rank == gameBetRanks.total_users -2"
                        class="fa-solid fa-poo mr-1" style="color:chocolate"></i>
              ...
              <div v-if="gameRoomUrl" class="text-center">
                <div v-show="showRoomInvitation" style="background: #00000042">
                  ...
                  <h2 class="d-flex justify-content-end">
                    <button class="btn btn-sm btn-secondary text-white mr-3" @click="toogleRoomInvitation">
                  ...
                  <div class="row">
                    <div class="col-12">
                      ...
                      <copy-link placement="right" heading-tag="h4"
                        custom-class="btn btn-outline-dark btn-sm text-white white-space-no-wrap"
                        id="host-game-room-url" :url="gameRoomUrl" :text="$t('Copy')"
                        :after-copy-text="$t('Copied link')"></copy-link>
                    </div>
              ...
              <div v-if="isBetGameHost" class="d-flex position-absolute mx-2 mb-4" style="right: 15px; bottom:0">
                <div v-show="!showRoomInvitation" class="btn btn-outline-dark users-toggle"
                ...
                <div>
                  <button v-if="!showGameRoomVotes" style="min-width: 45px"
                    class="btn btn-outline-dark ml-1 white-space-no-wrap black-box-toggle"
                    @click="toggleShowGameRoomVotes">
                    <h5>
                      <i class="fa-solid fa-box"></i>&nbsp;@{{ $t('game_room.black_box') }}
                    </h5>
                  </button>
                  <button v-else style="min-width: 45px"
                    class="btn btn-outline-dark ml-1 white-space-no-wrap black-box-toggle"
                    @click="toggleShowGameRoomVotes">
                    <h5>
      ...
      <div v-if="game && gameSerial && !finishingGame && !isMobileScreen" class="d-flex justify-content-end py-2"
        id="create-game">
        <create-game-room game-room-route="{{ route('game.room.index', '_serial') }}"
          get-room-endpoint="{{ route('api.game-room.get', '_serial') }}"
          create-game-room-endpoint="{{ route('api.game-room.create') }}" :get-game-serial="getGameSerial"
          :handle-created-room="handleCreatedRoom" :get-current-candidates="getCurrentCandidates"
          :has-active-room="runInBackGameRoom" :online-users="gameOnlineUsers"></create-game-room>
      </div>
      ...
      <div v-if="game && gameSerial && !finishingGame && isMobileScreen" class="position-fixed" id="create-game-mobile"
        style="right:10px; bottom:15px; z-index:1050" {{-- z-index:1050 to prevent modal mock in front of modal --}}>
        ...
        <div :class="{ 'modal-dialog': true, 'modal-lg': !isMobileScreen }">
          <div class="modal-content">
              ...
              <div class="d-flex justify-content-between w-auto">
                <div>
                  <share-link id="{{ $post['serial'] }}" url="{{ route('game.show', $post['serial']) }}"
                    text="{{ __('Share') }}" after-copy-text="{{ __('Copied link') }}"
                ...
                <form @submit.prevent="accessGame">
                  <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control font-size-16" v-model="inputPassword" autocomplete="off"
                      @input="knownIncorrectPassword = false">
                    <div class="input-group-append">
                      <button class="btn btn-primary" type="submit"
                        :disabled="inputPassword.length == 0 || knownIncorrectPassword">
                ...
                <div class="row no-gutters">
                  ...
                  <div class="col-6 px-1 pt-2">
                    <div class="comparison-media-box">
                      {{-- 1. 背景模糊層 --}}
                      <div v-if="post.element1.previewable" class="comparison-media-box__bg"
                    ...
                    <h2 class="text-center mt-2 p-1 element-title text-truncate">@{{ post.element1.title }}</h2>
                  </div>
                  ...
                  <div class="col-6 px-1 pt-2">
                    <div class="comparison-media-box">
                      {{-- 1. 背景模糊層 --}}
                      <div v-if="post.element2.previewable" class="comparison-media-box__bg"
                    ...
                    <h2 class="text-center mt-2 p-1 element-title text-truncate">@{{ post.element2.title }}</h2>
                  </div>

                  {{-- 下方資訊區塊 (保持不變) --}}
                  <div class="card-body pt-0 text-center mt-2">
                    <h5 class="text-break">{{ $post->description }}</h5>

                    <div v-if="post.tags && post.tags.length > 0" class="d-flex flex-wrap justify-content-center">
                      <span class="badge badge-secondary m-1" v-for="tag in post.tags" :key="tag"
                        style="font-size:medium">#@{{ tag }}</span>
                    </div>

                    <span class="mt-2 card-text d-flex justify-content-end align-items-center">
                      <span class="pr-2">
            ...
            <div class="modal-footer mb-sm-0 mb-4">
              <button v-if="post && elementsCount > 0" @click="createGame" type="submit"
              ...
              <b-popover :show.sync="showPopover" ref="select-element-count-hint"
                target="select-element-count-hint-target"
                placement="top">{{ __('game.select_option_hint') }}</b-popover>
            </div>
          </div>
        </div>
      </div>
