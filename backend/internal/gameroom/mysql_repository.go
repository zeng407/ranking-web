...
const roomBySerialQuery = `SELECT id, serial FROM game_rooms WHERE serial = ?`

func (repository *MySQLRepository) RoomBySerial(ctx context.Context, serial string) (Room, error) {
	var room Room
	err := repository.database.QueryRowContext(ctx, roomBySerialQuery, serial).Scan(&room.ID, &room.Serial)
...
func (repository *MySQLRepository) SettleBets(ctx context.Context, outcome BetOutcome) (SettleResult, error) {
	...
	won, err := transaction.ExecContext(ctx, settleWinnersStatement,
		repository.scoring.ComboScore, repository.scoring.WonScore,
...
//
// The streak, and therefore the combo bonus in each wager's payout, is DERIVED here rather
// than read from the row the settlement wrote. See Tally for why: last_combo is resolved
// when a wager is placed, from the wager before it, and that is only right if the previous
// one had already settled — which a player betting faster than the host votes cannot rely
// on. Deriving it makes the bonus a function of which wagers won, in round order, and
// nothing else.
func (repository *MySQLRepository) RecomputeTotals(ctx context.Context, roomID int64) (int64, error) {
	result, err := repository.database.ExecContext(ctx, recomputeTotalsStatement,
		// The payout expression, then the room, then the starting score, then the room
		// again for the outer WHERE.
		repository.scoring.ComboScore, repository.scoring.WonScore, repository.scoring.LoseScore,
		roomID, repository.scoring.DefaultScore, roomID)
	if err != nil {
		return 0, fmt.Errorf("gameroom: recompute totals for room %d: %w", roomID, err)
	}
