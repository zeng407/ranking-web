...
function localizedPublicRoutes(): RouteRecordRaw[] {
  ...
  return [
    ...
    // The Blade export page was a separate URL because the image was generated by a
    // separate page. Here the same generator runs from the result screen, so the old
    // link is a way into that screen and nothing more.
    {
      path: `${prefix}/r/:serial/export`,
      redirect: (to) => redirectToPath(to, localizedPath(`/r/${to.params.serial}`, normalizeLocale(to.params.locale))),
    },
