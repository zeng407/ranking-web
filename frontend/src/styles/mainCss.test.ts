...
describe('mobile touch targets', () => {
  it('grows header controls and carousel dots to a usable size', () => {
    ...
    expect(mobile).toMatch(/\.highlight-dots :is\(button, span\) \{[\s\S]*?padding:\s*1rem 0\.85rem/)
  })
  ...
  it('keeps the carousel dots painted inside their padded hit box', () => {
    ...
    // filled the whole hit box as a 53x43px block instead of staying a dot. The rule
    // covers spans too, so the loading placeholder is the same height as the real row.
    expect(rule('.highlight-dots :is(button, span)')).toContain('background-color: var(--border-strong)')
    expect(rule('.highlight-dots button.active')).toContain('background-color: var(--accent)')
    expect(css).not.toMatch(/\.highlight-dots :is\(button, span\)[^{]*\{[^}]*background:\s/)
  })
})
